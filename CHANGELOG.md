...
- MCP tool descriptions rewritten to say *when* to use each tool and what comes back (following the TDQS tool-definition guidance); `mmu_vibecheck` and `mmu_validate_idea` are listed first.
- Plugin manifest category `productivity` → `security`; description leads with the commit gate.
